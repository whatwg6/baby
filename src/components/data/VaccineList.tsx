  ...
  return (
    <section className="rounded-card border border-line bg-white p-4 shadow-panel sm:p-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <p className="shrink-0 rounded-card bg-mist px-3 py-1 text-sm font-medium text-primary">{vaccines.length} 条</p>
      </div>

      <div className="mt-4 space-y-2">
        {vaccines.length > 0 ? (
          vaccines.map((record) => (
            <article key={record.id} className="rounded-card border border-line bg-mist/70 p-3">
              <div className="flex items-start justify-between gap-3">
                ...
                {record.payload.dose ? (
                  <p className="shrink-0 rounded-card border border-line bg-white px-2 py-1 text-xs font-medium text-muted">
                    {record.payload.dose}
                  </p>
                ) : null}
              </div>
