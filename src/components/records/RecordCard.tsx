...
export function RecordCard({ record }: RecordCardProps) {
  ...
  return (
    <article className="rounded-card border border-line bg-white p-4 shadow-sm transition hover:border-primary/30 hover:shadow-soft">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            <time className="shrink-0 rounded-card bg-mist px-2 py-1 text-xs font-medium text-muted" dateTime={record.occurredAt}>
              {timeLabel}
            </time>
          </div>
          ...
          {record.mediaIds && record.mediaIds.length > 0 ? (
            <div className="mt-3 h-16 w-16 rounded-card border border-line bg-mist" aria-label="照片占位" />
          ) : null}
        </div>
      </div>
    </article>
